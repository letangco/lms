...
import { getSha1 } from '../../helpers/string.helper';

const parser = new xml2js.Parser({ explicitArray: false });

export async function roomRequest(apiOptions) {
  try {
    const result = await request(apiOptions);
    const res = await parser.parseStringPromise(result);
    return res?.response;
  } catch (error) {
    logger.error('roomRequest error');
    logger.error(error);
    throw error;
  }
}
...
function urlParamsFor(param) {
  switch (param) {
    case 'create':
      return [
        ['meetingID', true], ['name', true], ['attendeePW', false], ['moderatorPW', false], ['welcome', false], ['dialNumber', false],
        ['voiceBridge', false], ['webVoice', false], ['logoutURL', false], ['maxParticipants', false], ['record', false],
        ['logo', false], ['duration', false], ['moderatorOnlyMessage', false], ['autoStartRecording', false],
        ['allowStartStopRecording', false], ['bannerText', false], ['bannerColor', false], ['copyright', false], ['muteOnStart', false], ['guestPolicy', false],
        [/meta_\w+/, false],
      ];
    case 'join':
 ...
 * @param {String|optional} endpoint The server endpoint
 * @returns {string}
 */
export function getUrlForRoomCall(method, params = null, filter = true, endpoint = null) {
  let query = '';
  let sep;
  logger.logX(params);
  ...
  return `${endpoint || ROOM_ENDPOINT}/${query}`;
}
